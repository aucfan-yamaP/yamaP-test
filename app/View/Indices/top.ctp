<form action="update" method="post" enctype="multipart/form-data" name="cal_form" id="cal_form">
    ...
    <div class="menu_list">
        ...
        <?php if($view_only): ?>
            <ul class="shift_info">
                <li>●シフト時間</li>
                <?php foreach(Configure::read('MAO_SHIFTS') as $key => $val): ?>
    ...
    <table class="main_table" data-nextcal="<?php echo $after_y.'-'.$after_n; ?>" data-prevcal="<?php echo $before_y.'-'.$before_n; ?>">
        <tr class="week_tr">
            <?php foreach(Configure::read('WEEKEND_ENG') as $key => $val): ?>
    ...
    <div class="shift_box">
        <div class="contents radius">
            ...
            <ul>
                <?php foreach(Configure::read('MAO_SHIFTS') as $shift_name => $info): ?>
